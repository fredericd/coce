...
const validateCallback = (callback, requestLogger) => {
  if (!callback) {
    return { valid: true };
  }

  if (!/^[a-zA-Z_$][a-zA-Z0-9_$]*$/.test(callback)) {
    requestLogger.warn('Invalid callback parameter', { callback });
    return {
      valid: false,
        ...
        if (!includeAll) {
          ...
          Object.keys(result).forEach((id) => {
            ...
            const firstProvider = providers.find(
              (provider) => urlPerProvider[provider] !== undefined,
            );
...
app.get('/set', (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // URL is valid, continue with the validated URL if needed
      logger.debug('URL validation successful', { originalUrl: url, validatedUrl: validatedUrl.href });
    } catch (urlError) {
